...
class ApiService : Service() {
    ...
    companion object {
        ...
        private const val CHANNEL_ID      = "agentapi_service"
        ...
        const val ACTION_START   = "com.agentapi.android.START"
        const val ACTION_STOP    = "com.agentapi.android.STOP"
    ...
    override fun onCreate() {
        ...
        wakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "AgentAPI::WakeLock")
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ...
            else        -> startup()  // ACTION_START, null (restarted by OS), anything else
        }
    ...
    private fun startup() {
        if (isRunning) {
            Log.d(TAG, "Already running — refreshing notification")
        ...
        try {
            ...
            Log.i(TAG, "AgentAPI started — http://127.0.0.1:${AppConfig.getPort(this)} (loopback only)")
        } catch (e: Exception) {
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(CHANNEL_ID, "AgentAPI Service",
                NotificationManager.IMPORTANCE_LOW).apply {
                description = "AgentAPI HTTP server"
                setShowBadge(false)
    ...
    private fun buildNotification(): Notification {
        val mainIntent = PendingIntent.getActivity(
            this, 0, Intent(this, DeviceApiActivity::class.java),
        ...
        return NotificationCompat.Builder(this, CHANNEL_ID)
            .setContentTitle("AgentAPI Running")
