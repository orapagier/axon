...
object AppConfig {
    ...
    private const val PREFS_NAME = "androidcompanion_config"
    ...
    /** axon-agent webhook URL. e.g. https://your-axon-server.example.com/webhook/androidcompanion */
    ...
    // ── Device control on/off (server + tunnel, one switch) ─────────────────
    // Persisted so a user who turns this off stays off across reboots (BootReceiver)
    // and the 15-minute watchdog (WatchdogWorker) — both would otherwise resurrect
    // ApiService unconditionally. Default true: matches the pre-existing always-on
    // boot behavior for anyone who never touches the new toggle.

    fun isDeviceControlEnabled(ctx: Context): Boolean =
        prefs(ctx).getBoolean(KEY_DEVICE_CONTROL_ENABLED, true)

    fun setDeviceControlEnabled(ctx: Context, v: Boolean) =
        prefs(ctx).edit().putBoolean(KEY_DEVICE_CONTROL_ENABLED, v).apply()
}
