...
class StreamingTts(
    ...
    /** Drop everything immediately (a cancelled run, or a fresh reply taking
     *  over). Does not fire onDone. */
    fun abort() {
        abandoned = true
        ...
        try {
            ...
            // cancelled run calls abort(). This is the common case, not an
            // error: it used to crash with RejectedExecutionException. No
            // synth work is in flight to sequence behind, so stop playback
            // directly.
            player.abortStream(stream)
        }
    }
