# Device Control (AgentAPI) — axon-agent Integration Guide
...
**Note:** wiring this into axon-agent so it actually reacts to these events (rather
than just receiving them) needs a receiving endpoint on the axon-agent side — that's
separate, not-yet-built work, independent of Step 3 below (which covers axon-agent
*calling* the phone, the more commonly used direction).
...
1. In the Axon dashboard, save a credential:
   ```json
   {
     "name": "AgentAPI phone",
     "service": "agentapi",
     "data": { "base_url": "https://phone.yourdomain.com", "bearer_token": "YOUR_TOKEN" }
   }
   ```
2. Pre-save a named Synapse per action you actually want the agent to take, e.g.:
   - **"phone: send SMS"** → `credential: "agentapi"`, `method: POST`, `url: /agent/tool`,
     `json_body: {"tool":"sms.send","params":{"number":"{{number}}","message":"{{message}}"}}`,
     `timeout_seconds: 120`
   - **"phone: check battery"** → same shape, `{"tool":"battery.get","params":{}}`
   - **"phone: run shell command"** → `{"tool":"shell.run","params":{"command":"{{command}}"}}`
     — remember this one always waits for an on-device approval tap (see below).

   `timeout_seconds: 120` matters: some tools block up to 90 seconds waiting on an
   on-device Approve/Deny tap (see Security model), and the default HTTP timeout is 30s.
...
- **File sandbox:** `/sdcard/AgentAPI/` is the safe zone. Reads (`GET /files`,
...
```
Synapse "phone: send SMS" → credential: agentapi → POST /agent/tool
  {"tool":"sms.send","params":{"number":"{{number}}","message":"{{message}}"}}
```

**Ask Axon "what's my phone's battery at?"**
```
Synapse "phone: check battery" → credential: agentapi → POST /agent/tool
...
→ Must be enabled in: Settings → Accessibility → AgentAPI Auto Answer.
