...
└──────────────────────────────┬──────────────────────────────────────┘
                    ...
                    │         │          │
                    │  ┌──────▼───────┐  │
                    │  │  Watcher     │  │ ◄── Smart Notifications (polls email/cal/FB)
                    │  └──────────────┘  │
                    └────────┬───────────┘
...
### Smart Notifications
- **Automated polling** — Monitors Gmail, Outlook, Facebook, Google Calendar, and Outlook Calendar
- **AI-powered triage** — Uses a fast LLM to filter noise from signal (skips promos, newsletters, automated notifications)
- **Deduplication** — Tracks seen item IDs to never double-notify
- **Quiet hours** — Configurable do-not-disturb window
- **Multi-platform delivery** — Sends important notifications to Telegram, Discord, or Slack
- **Per-service intervals** — Check email every 5 min, calendar every 15 min, etc.
...
- **Watcher management** — Configure smart notification watchers per service
...
│   │   ├── watcher/
│   │   │   └── engine.rs          # Smart Notifications polling + triage
│   │   ├── tools/
...
The watcher engine automatically polls your services and uses AI to filter out noise:

```
Every N minutes:
  Poll Gmail, Outlook, Facebook, Calendars
  → Deduplicate (skip already-seen items)
  → Batch all new items into one triage prompt
  → AI filters: skip promos, newsletters, GitHub bots
  → Send digest of important items to Telegram/Discord/Slack
```

**Triage rules** (what gets filtered out):
- Promotional/marketing emails, newsletters
- Automated notifications (GitHub, Jira, CI/CD, Vercel)
- Social engagement (likes, follows)
- Spam, phishing, app updates

**What always gets through:**
- Personal/work emails from real people
- Client & business communications
- Payment/billing alerts, security alerts
- Calendar events starting within 30 minutes
- Direct customer messages (Facebook Messenger)

**Configuration** (via Dashboard → Settings):

| Setting | Default | Description |
|---|---|---|
| `watcher.enabled` | `false` | Master switch |
| `watcher.notify_platform` | `telegram` | Where to send notifications |
| `watcher.notify_chat_id` | *(empty)* | Chat/channel ID for notifications |
| `watcher.quiet_hours_start` | `22:00` | Quiet hours start (no notifications) |
| `watcher.quiet_hours_end` | `07:00` | Quiet hours end |
| `watcher.timezone_offset_hours` | `8` | UTC offset (8 = PHT, -5 = EST) |

**Adding watchers** (via API or Dashboard):
```bash
curl -X POST http://localhost:3000/api/watchers \
  -H 'Content-Type: application/json' \
  -d '{"service": "gmail", "poll_mins": 5, "enabled": true}'
```

**Built-in presets:** `gmail`, `outlook`, `facebook`, `gcal`, `mscal`

**Custom watchers** — watch ANY tool without code changes:
```bash
# Watch a custom MCP tool (e.g., a CRM server)
curl -X POST http://localhost:3000/api/watchers \
  -H 'Content-Type: application/json' \
  -d '{"service": "custom", "tool_name": "crm_list_leads", "tool_args": "{\"status\": \"new\"}", "label": "CRM Leads", "poll_mins": 10}'

# Watch an HTTP endpoint
curl -X POST http://localhost:3000/api/watchers \
  -H 'Content-Type: application/json' \
  -d '{"service": "custom", "tool_name": "web_request", "tool_args": "{\"url\": \"https://api.example.com/alerts\", \"method\": \"GET\"}", "label": "API Alerts", "poll_mins": 1}'
```

**Model role:** Assign a fast model with `role = "watcher"` in your models config. Falls back to general pool → paid model on error.
...
| `quality_checker` | Response validation | Gemini Flash Lite |
| `watcher` | Smart notification triage | Gemini Flash Lite |
...
| Method | Endpoint | Description |
|---|---|---|
| `GET` | `/api/watchers` | List all configured watchers |
| `POST` | `/api/watchers` | Create or update a watcher |
| `PUT` | `/api/watchers/:id` | Toggle watcher enabled/disabled |
| `DELETE` | `/api/watchers/:id` | Delete a watcher |
| `GET` | `/api/watchers/log` | View recent polling log |
