...
: "${AXON_TARGET_SERVER:?Set AXON_TARGET_SERVER (user@host) in .deploy.env — see .deploy.env.example}"
: "${AXON_REMOTE_DIR:?Set AXON_REMOTE_DIR in .deploy.env — see .deploy.env.example}"
TARGET_SERVER="${AXON_TARGET_SERVER//$'\r'/}"
...
ssh $SSH_OPTS "$TARGET_SERVER" "rm -f $REMOTE_DIR/$DEPLOY_FILE"
...
ssh $SSH_OPTS "$TARGET_SERVER" "bash -s" <<REMOTE
    ...
    # ── Save auth tokens before wipe (skip database for fresh start) ──
    echo "  🔑 Backing up auth tokens..."
    BACKUP_DIR="/tmp/axon_tokens_\$\$"
    mkdir -p "\$BACKUP_DIR"
    ...
    # ── Wipe everything for a completely fresh start ──
    echo "  🧹 Wiping $REMOTE_DIR/axon/ (fresh database)..."
    ...
    # ── Restore auth tokens only (fresh database) ──
    echo "  🔑 Restoring auth tokens..."
    # Integrations run in-process now: restore OAuth creds/tokens into core/.
