...
/// Spawns a background task that periodically pings available models to verify
/// their health. If a model times out or returns a rate-limit/server error,
/// it is proactively marked as unavailable (blacklisted) *before* a user request
/// attempts to use it.
pub async fn start_health_checker(
    ...
    let mut interval = tokio::time::interval(Duration::from_secs(
        settings.model_health_check_interval_secs(),
    ));
    loop {
        interval.tick().await;
        ...
        let models_to_ping = {
            let mut g = router.lock().await;
            // Free up models that have passed their cooldown
            for m in g.models.iter_mut() {
                if (m.status == "rate_limited" || m.status == "unavailable") && m.is_available() {
                    m.status = "available".into();
                    m.rate_limit_reset_at = None;
            ...
            // Group available models by provider to avoid pinging the same provider
            // redundantly. Pick one random available model per provider.
            let mut by_provider: std::collections::HashMap<String, Vec<usize>> =
                std::collections::HashMap::new();
            for (i, m) in g.models.iter().enumerate() {
                if m.enabled && m.is_available() && m.role != "paid_model" {
                    by_provider.entry(m.provider.clone()).or_default().push(i);
                }
            }

            let mut targets = Vec::new();
            for (_, indices) in by_provider {
                if let Some(&mi) = indices.first() {
                    targets.push(mi);
                }
            }
            targets
        ...
        if models_to_ping.is_empty() {
            continue;
        }

        for mi in models_to_ping {
            let (mut model_clone, timeout) = {
                let g = router.lock().await;
                if mi >= g.models.len() {
                    continue;
                }
                ...
                let mut c = m.clone();
                // FIX: Support comma-separated fallback model IDs for the health check.
                // We only need to ping the primary (first) model ID to verify the endpoint is alive.
                c.model_id = c
                    .model_id
                    .split(',')
                    .next()
                    ...
                    .trim()
                    .to_string();
                c.api_key = settings.resolve(&m.api_key);
                (c, m.timeout_secs.unwrap_or(5))
            };

            let m_name = model_clone.name.clone();

            // Spawn each ping so one slow provider doesn't block the health checker.
            let router_ref = router.clone();
            let pinger = async move {
                // We prepare a minimal tiny payload just to test connectivity.
                let messages = vec![crate::providers::types::Message::user("hi")];

                // Short timeout for health checks — if it can't respond to "hi"
                // in 5 seconds (or whatever its set timeout is), it's congested.
                let deadline = tokio::time::sleep(std::time::Duration::from_secs(timeout.min(5)));
                let call = crate::providers::call_provider(
                    &mut model_clone,
                    &messages,
                    "Short health check answer.",
                    &[],
                    5, // request tiny output
                );

                tokio::select! {
                    _ = deadline => {
                        let mut g = router_ref.lock().await;
                        if mi < g.models.len() {
                            tracing::warn!("Health check: '{}' timed out, putting on cooldown", m_name);
                            g.models[mi].mark_error(1, 2); // 1 threshold, 2 min cooldown
                        }
                    }
                    res = call => {
                        let mut g = router_ref.lock().await;
                        if mi < g.models.len() {
                            match res {
                                Ok(_) => {
                                    // Successfully pinged.
                                    g.models[mi].consecutive_errors = 0;
                                }
                                Err(e) => {
                                    let err_str = e.to_string();
                                    tracing::warn!("Health check: '{}' failed: {}", m_name, err_str);
                                    if err_str.contains("429") || err_str.contains("rate limit") {
                                        g.models[mi].mark_rate_limited(2);
                                    } else {
                                        g.models[mi].mark_error(1, 2);
                                    }
                                }
                            }
                        }
