...
const CATEGORY_META = {
  ...
  tts: { title: 'Voice Replies', description: 'Text-to-speech that speaks agent replies on the Chat page — after a voice message, the answer is read back in this voice. Set a base URL (Groq: https://api.groq.com/openai/v1; OpenAI: https://api.openai.com/v1; Gemini: https://generativelanguage.googleapis.com/v1beta/openai — Gemini has no OpenAI-style speech route, so it is served through its native speech API automatically), a model, and a voice name (Gemini voices: Kore, Puck, Zephyr, …). Leave blank — or if the endpoint errors or is rate-limited — and the dashboard falls back to the browser’s built-in speech synthesis. Applies immediately, no restart needed.' },
