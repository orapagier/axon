...
impl MicrosoftService {
    ...
    pub fn tool_list() -> Vec<Tool> {
        vec![
            ...
            Tool::new("mscal_list_events", "List Microsoft Calendar events. Use this when asked about the Outlook/Microsoft calendar or schedule. Supports free-text search via 'query'. Note: 'calendar_id' and 'query' cannot be used together — if both are supplied an error is returned.\n\n**Time windows:**\n- Passing a date-only `start_datetime` (e.g. '2026-07-20') with no `end_datetime` lists ONLY that single calendar day and filters events to that day (see `_axon_requested_day` in the response).\n- For a range or far-future dates, pass BOTH `start_datetime` and `end_datetime`.\n- With no window at all, listing covers now through the next 30 days.\n- `query` searches ALL events regardless of any time window — do not rely on start/end when `query` is set.", schema!({"max_count":{"type":"integer","default":10,"description":"Max events to return (up to 50)"},"start_datetime":{"type":"string","description":"Window start. Any common datetime format works: ISO 8601, '2026-07-05 09:00', 'July 5, 2026 3pm', or a Unix timestamp. A bare date (e.g. '2026-07-20') scopes to that exact day only.","displayOptions":{"inlineGroup":"time_window"}},"end_datetime":{"type":"string","description":"Window end. Accepts the same flexible formats as start_datetime.","displayOptions":{"inlineGroup":"time_window"}},"query":{"type":"string","description":"Free-text search. Cannot be combined with calendar_id."},"calendar_id":{"type":"string","description":"Specific calendar ID. Cannot be combined with query."}}, [])),
