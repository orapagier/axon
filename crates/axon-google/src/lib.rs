...
impl GoogleService {
    ...
    pub fn tool_list() -> Vec<Tool> {
        let mut tools = vec![
            ...
            // Gmail
            Tool { name: "gmail_list".into(),       description: "List Gmail messages. Returns id, subject, from, date, snippet. Use this when asked to check email, the inbox, or unread mail (query 'is:unread').".into(),          input_schema: schema!({"max_results":{"type":"integer","default":10,"description":"Max messages (max 10)"},"query":{"type":"string","description":"Gmail search query, e.g. 'is:unread from:boss@co.com'"}}, []) },
            ...
            Tool { name: "gcal_list_events".into(),  description: "List Google Calendar events. Use this when asked about the calendar, schedule, agenda, or upcoming events. Supports free-text search via 'query' parameter. Set 'single_events' to false to discover master events of recurring series (required for series deletion). When 'single_events' is true and no 'time_min' is given, listing starts from now. Responses with more events than 'max_results' include a nextPageToken — pass it back as 'page_token' for the next page.".into(),                                  input_schema: schema!({"max_results":{"type":"integer","default":10,"maximum":2500},"time_min":{"type":"string","description":"ISO 8601 start time","displayOptions":{"inlineGroup":"time_window"}},"time_max":{"type":"string","description":"ISO 8601 end time","displayOptions":{"inlineGroup":"time_window"}},"query":{"type":"string","description":"Free-text search terms"},"calendar_id":{"type":"string","default":"primary"},"single_events":{"type":"boolean","default":true},"page_token":{"type":"string","description":"nextPageToken from a previous response, to fetch the next page"}}, []) },
            Tool {
            ...
            Tool { name: "gcal_create_event".into(), description: "Create a Google Calendar event. Defaults to 'Asia/Manila' timezone. Set 'create_meet_link' to true to generate a Google Meet link. For recurring events, provide RRULE strings in the 'recurrence' array (e.g. ['RRULE:FREQ=WEEKLY;BYDAY=FR']). For an ALL-DAY event pass dates only (e.g. start '2025-06-15', end '2025-06-15').".into(),
                input_schema: schema!({
                    "summary":         { "type": "string",  "description": "Event title / name (SUMMARY). What the event is called, e.g. 'Team Standup' or 'Doctor Appointment'." },
                    "start":           { "type": "string",  "description": "Start date and time in ISO 8601 format, e.g. '2025-06-15T09:00:00'. Include date AND time for timed events; a date alone ('2025-06-15') makes an all-day event.", "displayOptions": { "inlineGroup": "event_time" } },
                    "end":             { "type": "string",  "description": "End date and time in ISO 8601 format, e.g. '2025-06-15T10:00:00'. Must be after start. For all-day events use a date; same date as start means a one-day event.", "displayOptions": { "inlineGroup": "event_time" } },
            ...
            // Drive
            Tool { name: "gdrive_list".into(),         description: "List Google Drive files/folders. Use this when asked what files or folders are in Drive.".into(),                                      input_schema: schema!({"max_results":{"type":"integer","default":10},"folder_id":{"type":"string"},"mime_type":{"type":"string"}}, []) },
            ...
            Tool { name: "gsheets_batch_read".into(), description: "Read multiple ranges in a single request.".into(), input_schema: schema!({"spreadsheet_id":{"type":"string"},"ranges":{"type":"array","description":"Ranges to read, one per row.","items":{"type":"object","properties":{"range":{"type":"string","description":"e.g. Sheet1!A1:C10"}}}}}, ["spreadsheet_id","ranges"]) },
