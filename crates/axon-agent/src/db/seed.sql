...
INSERT OR IGNORE INTO settings VALUES
    ...
    ('memory.min_similarity_pct',   '25',    'int',    'Minimum similarity (%) for a memory to be recalled; below this recall returns nothing rather than injecting weak matches', 'memory', datetime('now')),
    ('memory.dedup_similarity_pct', '95',    'int',    'Similarity (%) at which a new memory refreshes an existing one from the same source instead of inserting a duplicate (0=disable)', 'memory', datetime('now')),
    ('memory.vector_scan_limit',    '2000',  'int',    'Recent embedded memories cosine-scanned per recall so semantically relevant memories are found even when they share no wording with the query (0=keyword-only)', 'memory', datetime('now')),
...
INSERT OR IGNORE INTO settings VALUES
    ('tts.base_url', '', 'string', 'Speech base URL (Groq: https://api.groq.com/openai/v1, OpenAI: https://api.openai.com/v1, Gemini: https://generativelanguage.googleapis.com/v1beta/openai — Gemini is auto-served via its native speech API; type the literal word "piper" for a free offline local voice — run deploy/setup-piper.sh to install it). Blank disables spoken replies — the dashboard falls back to the browser''s built-in voice.', 'tts', datetime('now')),
    ('tts.model',    '', 'string', 'Speech-synthesis model. Pick from the dropdown (prefetched from tts.base_url''s /models catalogue) or type any ID (e.g. playai-tts on Groq, gpt-4o-mini-tts on OpenAI, gemini-2.5-flash-preview-tts on Gemini, or an installed voice like en_US-lessac-medium when tts.base_url is "piper").', 'tts', datetime('now')),
    ...
    ('tts.piper.length_scale',    '', 'float', 'Piper only: speaking pace (--length_scale, model default 1.0). Above 1 slows down, which usually sounds more natural — try 1.05–1.12; below 1 speeds up. Blank keeps the voice''s own default.', 'tts', datetime('now')),
    ('tts.piper.noise_scale',     '', 'float', 'Piper only: expressiveness / generator noise (--noise_scale, model default 0.667). Slightly higher adds pitch variation and life; too high gets warbly. Blank keeps the default.', 'tts', datetime('now')),
    ('tts.piper.noise_w',         '', 'float', 'Piper only: cadence variability, i.e. phoneme-width noise (--noise_w, model default 0.8). Higher varies rhythm/timing between words. Blank keeps the default.', 'tts', datetime('now')),
    ('tts.piper.sentence_silence','', 'float', 'Piper only: seconds of pause after each sentence (--sentence_silence, model default 0.2). Raise for more breathing room between sentences. Blank keeps the default.', 'tts', datetime('now'));
